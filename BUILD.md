...
> [!NOTE]
> This is a working draft. The **Bill of Materials** (exact parts/links) and **3D printing settings** are being finalized — the sections below are marked _TODO_ where your specifics go.
...
- **2× 3.7 V LiPo cells** (this build uses 1100 mAh each, wired in **parallel** for ~2200 mAh).
...
- **USB-C breakout with CC resistors** (e.g. SparkFun USB-C Breakout) — accessible charging port.
- **2× 10 kΩ resistors** — battery-level voltage divider _(optional; see [Battery monitor](README.md#battery-monitor-optional))_.
...
- [`3d-print/tacpad-body.stl`](3d-print/tacpad-body.stl) — the main body / enclosure.
- [`3d-print/tacpad-lid.stl`](3d-print/tacpad-lid.stl) — the lid.

> _TODO: print settings (material, layer height, infill, supports, orientation) and any finishing/painting notes._
...
Flash the firmware and copy the audio assets to the SD card — full steps in the [README → Build & flash](README.md#build--flash). On first boot, pair over **Bluetooth** and set your stratagem keybinding in settings.
